...
/* Structure to define command-line options */
typedef struct {
    int opttype;       // Option type (switch, value, argument)
    int vartype;       // Variable type (int, double, string, etc.)
    void *pt;          // Pointer to the value
    int  *count;       // Count for array type variables
    char *names;       // Names for the option (including short options like '-x')
    char *text;        // Descriptive text for the option
} OPT_STRUCT;
...
static void OPT_print_vars(FILE *fp, OPT_STRUCT *opt, char *prefix, int verbose) {
    ...
    while (opt->opttype) {
        ...
        if (opt->opttype == OPTTYPE_SWITCH) {
            ...
            switch (opt->vartype) {
                case VARTYPE_int:       fprintf(fp, " %d", *(int *)(opt->pt)); break;
                case VARTYPE_double:    fprintf(fp, " %f", *(double *)(opt->pt)); break;
                case VARTYPE_charS:
...
// Example of how to call the cmd line parser in main function:
