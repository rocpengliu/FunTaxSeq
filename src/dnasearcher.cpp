...
DNASearcher::DNASearcher(Options * & opt, BwtFmiDB* & bwtfmiDB, char db) {
    ...
    if(db == 'd'){
        ...
        mCommonOptions = opt->mHostSearchOptions->comOptions;
    } else if(db == 'm'){
        mCommonOptions = opt->mMarkerSearchOptions->comOptions;
    } else {
        error_exit("you must specify a db for DNA search!");
    }
    ...
    blosum62diag[nuc2int['A']] = 2;
    blosum62diag[nuc2int['C']] = 2;
    blosum62diag[nuc2int['G']] = 2;
    blosum62diag[nuc2int['T']] = 2;

    b62[nuc2int[(uint8_t) 'A']][nuc2int[(uint8_t) 'C']] = 0;
    b62[nuc2int[(uint8_t) 'A']][nuc2int[(uint8_t) 'T']] = 0;
    b62[nuc2int[(uint8_t) 'A']][nuc2int[(uint8_t) 'G']] = 1; //1

    b62[nuc2int[(uint8_t) 'C']][nuc2int[(uint8_t) 'A']] = 0;
    b62[nuc2int[(uint8_t) 'C']][nuc2int[(uint8_t) 'G']] = 0;
    b62[nuc2int[(uint8_t) 'C']][nuc2int[(uint8_t) 'T']] = 1;//1

    b62[nuc2int[(uint8_t) 'G']][nuc2int[(uint8_t) 'A']] = 1;//1
    ...
    b62[nuc2int[(uint8_t) 'T']][nuc2int[(uint8_t) 'C']] = 1;//1
...
void DNASearcher::getAllFragments(Read * & item) {
    ...
    if (mCommonOptions.mode == GREEDY) {
        uint32 score = calcScore(item->mSeq.mStr);
        if (score >= mCommonOptions.minScore) {
...
std::set<char *>& DNASearcher::dnaSearchWuNeng(Read* & item) {
    ...
    if (mCommonOptions.mode == MEM) {
        classify_length();
    } else if (mCommonOptions.mode == GREEDY) {
...
void DNASearcher::classify_greedyblosum() {
    ...
    while (1) {
        ...
        if (num_mm > 0) {
            ...
            si = maxMatches(mBwtfmiDB->fmi, seq, (uint32) length, mCommonOptions.seedLength, 0, mCommonOptions.db); //initial matches
        }
        ...
        if (mCommonOptions.misMatches > 0 && num_mm < mCommonOptions.misMatches) {
            SI *si_it = si;
            while (si_it) {
    ...
    if (mCommonOptions.useEvalue) {
        ...
        if (Evalue > mCommonOptions.minEvalue) {
...
void DNASearcher::eval_match_scores(SI *si, Fragment *frag) {
    ...
    } else if (score == best_match_score && best_matches_SI.size() < mCommonOptions.max_matches_SI) {
        best_matches_SI.push_back(si);
        if (mOptions->verbose)
            best_matches.push_back(frag->seq.substr(si->qi, si->ql));
    } else {
...
void DNASearcher::addAllMismatchVariantsAtPosSI(const Fragment *f, unsigned int pos, size_t erase_pos = std::string::npos, SI *si = NULL) {
    assert(mCommonOptions.mode == GREEDY);
    ...
    for (auto itv : blosum_subst.at(origchar)) {
        // we know the difference between score of original base and substitution score, this
        ...
        if (score_after_subst >= (int) best_match_score && score_after_subst >= (int) mCommonOptions.minScore) {
            if (UpdateSI(mBwtfmiDB->fmi, mBwtfmiDB->astruct->trans[(size_t) itv], siarray, siarrayupd, mCommonOptions.db) != 0) {
                fragment[pos] = itv;
            ...
            if (mOptions->debug) {
                fragment[pos] = itv;
                std::stringstream ss;
                ss << "Skipping fragment " << fragment << " and following fragments, because score is too low: " << score_after_subst << " < " << std::max(best_match_score, mCommonOptions.minScore);
                cCout(ss.str(), 'y');
            }
            break;
        }
...
void DNASearcher::ids_from_SI(SI *si) {
    ...
    for (k = si->start; k < si->start + si->len; ++k) {
        if (match_ids.size() > mCommonOptions.max_match_ids) {
            break;
        }
        get_suffix(mBwtfmiDB->fmi, mBwtfmiDB->bwt->s, k, &iseq, &pos, mCommonOptions.db);
        match_ids.insert(mBwtfmiDB->bwt->s->ids[iseq]);
    }
    if(match_ids.empty()){
        cCout("dna search ", match_ids.size(), 'g');
    }
}
