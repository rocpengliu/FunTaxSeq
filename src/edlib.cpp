...
/**
 * @param [in] block
 * @return Values of cells in block, start with bottom cell in block.
 */
...
/**
 * Writes values of cells in block into given array, start with first/top cell.
 * @param [in] block
...
/**
 * Writes values of cells in block into given array, start with last/bottom cell.
 * @param [in] block
 * @param [out] dest  Array into which cell values are written. Must have size of at least WORD_SIZE.
 */
...
static int myersCalcEditDistanceSemiGlobal(
    ...
    *numPositions_ = 0;
    ...
    for (int c = 0; c < targetLength; c++) { // for each column
        ...
        // column because start conditions at upper boundary are 0.
...
static int obtainAlignmentHirschberg(
    ...
    // scoresLeft contains scores from left column, start with scoresLeftStartIdx row (query index)
...
static string transformSequences(const char* const queryOriginal, const int queryLength,
    ...
    // Each letter is assigned an ordinal number, start from 0 up to alphabetLength - 1,
