...
PairEndProcessor::PairEndProcessor(Options* opt, BwtFmiDBPair* & bwtfmiDBPair){
    mOptions = opt;
...
bool PairEndProcessor::process(){
    ...
    for(int t=0; t<mOptions->thread; t++){
        configs[t] = new ThreadConfig(mOptions, mBwtfmiDBPair, t, true);
    ...
    preStats1.reserve(mOptions->thread);
    vector<Stats *> postStats1;
    postStats1.reserve(mOptions->thread);
    vector<Stats *> preStats2;
    preStats2.reserve(mOptions->thread);
    vector<Stats*> postStats2;
    postStats2.reserve(mOptions->thread);
    vector<FilterResult*> filterResults;
    filterResults.reserve(mOptions->thread);
    for (int t = 0; t < mOptions->thread; t++){
    ...
    mOptions->mHomoSearchOptions->totReads = finalPreStats1->getReads();
    mOptions->mHomoSearchOptions->totCleanReads = finalPostStats1->getReads();
    int *dupHist = NULL;
