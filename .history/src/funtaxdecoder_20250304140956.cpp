...
std::string FunTaxDecoder::decodeFun(std::unordered_set<std::string>& locSet) {
    ...
    if(!tmpSet2.empty()){
        if(treItSet.empty()){
            ...
            std::unordered_set<std::vector<int>, VectorHash> path_set;
            std::vector<int> path_vec;
            path_vec.reserve(10);
            for(const auto & itp : treItSet){
                path_vec.clear();
                path_vec = mPhyloTree->geneTree->path_from_iterator(itp, mPhyloTree->geneTree->begin());
                if(path_vec.size() > 3){
                    path_vec.erase(path_vec.begin() + 2, path_vec.end());
                }
                path_set.insert(path_vec);
            }
            for(const auto & itv : path_set){
                tree<std::string *>::pre_order_iterator locp = mPhyloTree->geneTree->iterator_from_path(itv, mPhyloTree->geneTree->begin());
                tree<std::string *>::pre_order_iterator endp = locp;
                ++endp;
                for(const auto & itp : tmpSet2){
