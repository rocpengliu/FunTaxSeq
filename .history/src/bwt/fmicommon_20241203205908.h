...
static FMI *read_fmi_common(int index2_size, FILE *fp) {
  ...
  while(bytes_read < f->bwtlen){
    printf("readed length is %", bytes_read);
    ...
    if (n < chunk_size){
      fprintf(stderr,"Error: fread failed for bwt\n");
      exit(1);
    }
  }

  //fread(f->bwt, sizeof(uchar), f->bwtlen, fp); // why can not read and killed.
  printf("f->bwt reading complete\n");
  ...
  printf("allocating memory for N2\n");
